...
Value add_str(int arg_n, Module* mod, Value* args) {
  ...
  return MAKE_STRING(mod->stack, new_str);
}
...
Value to_string(int arg_n, Module* mod, Value* args) {
  ...
  switch (get_type(args[0])) {
    ...
    case TYPE_LIST: {
      ...
      return MAKE_STRING(mod->stack, res);
    }
    case TYPE_SPECIAL: {
      return MAKE_STRING(mod->stack, "<special>");
    }
    case TYPE_FUNCTION: case TYPE_FUNCENV: {
      return MAKE_STRING(mod->stack, "<function>");
    }
    case TYPE_UNKNOWN: {
      return MAKE_STRING(mod->stack, "<unknown>");
    }

    case TYPE_THREAD: {
      return MAKE_STRING(mod->stack, "<thread>");
    }

    case TYPE_API: {
      return MAKE_STRING(mod->stack, "<api>");
    }
  }
...
Value list_append(int arg_n, Module* mod, Value* args) {
  ...
  return MAKE_LIST(mod->stack, new_list, hp->length + 1);
}
...
Value list_concat(int arg_n, Module* mod, Value* args) {
  ...
  return MAKE_LIST(mod->stack, new_list, hp1->length + hp2->length);
}
...
Value get_index_str(int arg_n, Module* mod, Value* args) {
  ...
  if (idx < 0 || idx >= hp->length) return make_none(mod);

  return make_some(mod, MAKE_CHAR(mod, str[idx]));
}
...
Value str_slice(size_t argc, Module* mod, Value* args) {
  ...
  return MAKE_STRING(mod->stack, slice);
}
...
Value ffi_slice_list(size_t argc, Module* mod, Value* args) {
  ...
  return MAKE_LIST(mod->stack, new_list, sz);
}