...
Value ffi_print(int arg_n, Module* mod, Value* args) {
...
Value ffi_println(int arg_n, Module* mod, Value* args) {
  if (arg_n < 1) THROW("Print expects at least 1 argument");
  Value v = args[0];
...
Value get_args(int arg_n, Module* mod, Value* args) {
  return MAKE_LIST(mod->stack, mod->argv, mod->argc);
}
...
Value ffi_get_index(int arg_n, Module* mod, Value* args) {
  ...
  if (idx < 0 || idx >= l->length) return make_none(mod);

  return make_some(mod, l->as_ptr[idx]);
}
...
Value input(int arg_n, Module* mod, Value* args) {
  ...
  return MAKE_STRING(mod->stack, buffer);
}
...
Value read_file(size_t argc, Module *mod, Value *args) {
  ...
  if (file == NULL) return make_none(mod);
  ...
  fclose(file);
  return make_some(mod, MAKE_STRING(mod->stack, contents));
}